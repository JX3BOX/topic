...
<!--            <img :src="imgurl+'nav/'+item+'.png'" class="u-img" :class="'u-nav-active'+item" v-for="item in 6" :key="'navimg'+item" v-show="item != navActive && (mouseoverActive!=item || mouseoverActive==null)" @click="goAnchor(item)" @mouseover="mouseover(item)" >-->
            <img :src="imgurl+'nav/'+item+'.png'" class="u-img" :class="['u-nav-active'+item,item==mouseoverActive?'u-img-contrast':'']" v-for="item in 6" :key="'navimg'+item" v-show="item != navActive" @click="goAnchor(item)" @mouseover="mouseover(item)" >
            <a><div @click="goAnchor(0)" class="u-nav"></div></a>
        </div>
		<!-- 第一屏 -->
